...
The shell is a living-room home: on Home the **linear galaxy** is the only focal point (packaged Three.js in WebView2 — original procedural art, no third-party plates). **Up** opens a translucent horizontal games list over the galaxy; **Down** opens Settings; **Left/Right** shift the nodes. Enter on a focused node opens that group (Games stays as overlay). Network, Files, Hardware, and Settings land on list UI. Settings and Profiles stay discreet Home corner glyphs. A quiet Home HUD can show local time and CPU/GPU temps; hide them in Settings. The old Settings tile that showed the letter “I” is gone.
...
| Module | Purpose |
|--------|---------|
| **Home galaxy** | Black-field Home: original linear galaxy. Up opens a translucent Steam library list (`steam://rungameid`) over the galaxy. Down opens Settings. Left/right pan nodes. Optional Home HUD: clock and temps. See [docs/cube-nav.md](docs/cube-nav.md) |
...
- Galaxy motion: star drift under node gravity, bloom, and a translucent games list on Up. Instant snap when Settings, Windows animations, or a live session say off
- Keyboard: Up games, Down Settings, Left/Right shift nodes, Enter opens. Mouse: drag / flick / click a node. Gamepad D-pad / A is mapped in Core for a later stub
- Atmosphere: Home is a filmic black field with a mysterious linear galaxy. Inner pages keep Unbound cyan / cobalt chrome. Circuit amber remains a seasoning on tiles
